...
const Edit = () => {
  ...
  const validateForm = () => {
    const newErrors = {
      ...
      price: price.trim() === "",
      images: existingImages.length - imagesToRemove.length + newImages.length === 0
    };
    setErrors(newErrors);
    return !newErrors.name && !newErrors.description && !newErrors.price && !newErrors.images;
  };
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (existingImages.length - imagesToRemove.length + newImages.length + newFiles.length > 10) {
      ...
      const existingNames = newImages.map(img => img.name);
      ...
      if (uniqueNewFiles.length > 0) {
        const updatedImages = [...newImages, ...uniqueNewFiles];
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await updateProduct({ productId: productId || "", formData: formData }).unwrap();
      setLoading(false);
    ...
    <div className="w-full max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="text-gray-500 mt-3 max-w-2xl mx-auto">
          Update the product details and images below.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100">
          ...
          <div className="mb-6">
            <div className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${
              ...
              existingImages.length - imagesToRemove.length + newImages.length > 0 ? 
              'border-indigo-300 bg-indigo-50' : 'border-gray-300 hover:border-indigo-200'
            }`}>
              <input
                id="imageUpload"
                type="file"
                ...
                className="hidden"
                accept="image/*"
                disabled={existingImages.length - imagesToRemove.length + newImages.length >= 10}
              />
              ...
              {existingImages.length - imagesToRemove.length + newImages.length === 0 ? (
                <label htmlFor="imageUpload" className="cursor-pointer block">
                  ...
                  <p className={`font-medium text-lg ${errors.images ? 'text-red-600' : 'text-indigo-600'}`}>
                    Drag & drop images here or click to browse
                  </p>
                  <p className={`text-sm mt-2 ${errors.images ? 'text-red-500' : 'text-gray-500'}`}>
                    Supports JPG, PNG, WEBP (Max 5MB each)
                  </p>
                  <button 
                    type="button"
                    className={`mt-4 px-6 py-2 rounded-lg transition-colors ${errors.images ? 'bg-red-100 text-red-600 border border-red-300' : 'bg-white border border-indigo-500 text-indigo-500 hover:bg-indigo-50'}`}
                  >
                    Select Files
                  </button>
                ...
                <div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 mb-6">
                    {/* Existing images that aren't marked for removal */}
                    {existingImages
                      .filter(img => !imagesToRemove.includes(img.publicId))
                      .map((img, index) => (
                    ...
                    {newImagePreviews.map((preview, index) => (
                      <div key={`new-${index}`} className="relative group">
                        ...
                        <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded">
                          {newImages[index].name.substring(0, 15)}{newImages[index].name.length > 15 ? '...' : ''}
                        </div>
                      </div>
                    ))}
                    
                    {/* Add more button if we haven't reached the limit */}
                    {existingImages.length - imagesToRemove.length + newImages.length < 10 && (
                      <label htmlFor="imageUpload" className="aspect-square border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition-colors group">
                        ...
                        <span className="text-gray-400 text-xs mt-1">
                          {10 - (existingImages.length - imagesToRemove.length + newImages.length)} remaining
                        </span>
                      </label>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between bg-indigo-50 rounded-lg p-4">
                    <p className="text-indigo-700 font-medium">
                      ...
                      {existingImages.length - imagesToRemove.length} existing image(s), {newImages.length} new image(s). First image will be used as featured.
                    </p>
                    <button 
                      type="button"
                      onClick={() => {
                        setImagesToRemove(existingImages.map(img => img.publicId));
                        setNewImages([]);
                        newImagePreviews.forEach(preview => URL.revokeObjectURL(preview));
                        setNewImagePreviews([]);
                      }}
        ...
        <div className="flex justify-end pt-6">
          <button
            ...
            className="px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:opacity-90 transition-opacity flex items-center shadow-lg"
          >
            {isLoading || loading ? (
              <>
